...
suspect they're wasting money, and I think they're correct.
...
fairly. doctl itself isn't important here. It's simply a convenient source of real data. The
method doesn't depend on doctl, on these six categories, or on DigitalOcean, and that
...
That isn't the recommendation I expected to make. I began with a plan to run a cheap model on
...
shown in the application is produced by the code here. I didn't calculate results separately
and paste them in.
...
offline simulator and isn't evidence. There are none of those left in this repository.

One thing matters more than any single number here, so I want it stated before the results
rather than buried after them. **A run isn't reproducible, even at temperature 0.** I ran the
...
work across requests, and that changes the arithmetic slightly, so identical inputs don't give
identical outputs.
...
open-weight. That means I have no direct comparison against a frontier model, and I'm not
claiming one. `qwen3.5-397b-a17b` was meant to stand in for whatever expensive model the customer
pays for today, and it couldn't: it returned one usable answer out of 109 and rate-limited the
rest. So I have no proxy for the frontier tier at all, and I say what follows without one.

That's less of a hole than it sounds. The customer's question is whether a cheaper model is good
...
284B model on a task, the task isn't what is limiting the result, and buying a larger model is
unlikely to change it.

I wasn't trying to test every available model. I was trying to cover a wide range, so that
whichever model won, I'd understand the reason it won. The eleven therefore span three
...
inexpensive. Where that's the case, I give both the total size and the part that actually runs,
...
and I checked them on 7 Aug 2026. They're written down in one file,
[`app/catalog.py`](app/catalog.py), and copied onto every saved result. If DigitalOcean changes
a price later, that change therefore can't quietly alter a conclusion I reached earlier.

**Models I chose not to include.** I left out `qwen3-coder-flash` because it's built for
...
left out the image, audio and video models because they don't perform this kind of task at all.
And I couldn't include Claude or GPT-5.x, because the exercise credits don't cover them.
...
can't separate them on the work that matters.**
...
and DeepSeek 12. A paired test on that gives **p = 1.00**. There's no accuracy difference here
at all.
...
So I recommend Mistral on speed, not on accuracy. It's 1.7 times faster at p50 and 2.6 times
faster at p95, and it held that lead across five dev runs and both test runs. That's a real
difference. Three points of macro-F1 resting on 13 issues isn't.
...
million classifications a month that's $81 against $239, and the $158 difference doesn't
justify giving up half your latency. At a hundred million it's $8,140 against $23,900, and the
argument reverses. The crossover is somewhere around ten million a month, and I'd move at
...
stop the workload. That isn't a hypothetical: during screening `qwen3.5-397b-a17b` failed 105
...
have now been settled, and two of them went against me. I've left them as they were rather
than quietly editing the prediction to match the outcome.

**I said I'd abandon the second opinion if the two models scored within 0.03 of each other.
They did, so I abandoned it.** That was the plan the whole design was built around: cheap model
on everything, hard issues escalated to a large one. It doesn't survive the measurements. The
top six models sit inside a 0.07 band that's no wider than one model's own run-to-run noise, so
there's no larger model here that's reliably better to escalate *to*. Worse, every one of the
...
Hard issues need a person or a better prompt, not more parameters. I'd run one model and
...
Against Mistral it's 0.041 behind on the dev split with no overlap across five runs each, so on
my own rule it doesn't qualify as an equal-quality substitute today. It becomes the right choice
on volume instead, for the cost reason above, and that's a different argument from the one I
...
its calls failed outright. Sorting six labels doesn't reward deliberation.

**I'm trading speed against volume.** Concurrency controls this: how many requests are in
flight at once. I expected the familiar curve, where throughput flattens, individual requests
slow down and rate-limit errors start. **I didn't find it.**
...
Throughput rose almost in step with concurrency across a 16x range, p95 didn't degrade, and
...
wall-clock against a 5.4 second slowest call. That's a burst, not sustained throughput, and
printing it in the same column as the lower numbers would invite a comparison it can't support.
...
measured at. A latency figure without the load that produced it can't be acted on.

**I'm trading the value of reasoning against the cost of reasoning.** I covered the figures
above: roughly 45 times the output cost, and a p95 latency measured in whole seconds, to choose
one word from a list of six. It's very unlikely to be worth paying for, and that's a finding
...
anything. There's no frontier model to compare against. Each of these is a genuine limitation,
...
own. The band is no wider than the noise, so I can't rank the models inside it, and the answer
to the customer's question isn't "buy the best one". Nothing here justifies paying more for
...
**Whether the small categories hold up. They don't, and not in the way I predicted.** I
...
everything**, because it's 121 of the 253 test issues and a model that's unsure drifts towards
...
`other` is worth one more sentence, because it's where I'd push back on my own headline
number. Three issues can't support a per-class F1, and the harness marks the class `thin` for
...
issues, and that's a limitation of my schema and my dataset size, not of the models.
...
timeout, which is why three calls failed outright. Choosing one word from a list of six doesn't
reward deliberation.

**Whether any model can't follow the output format. Almost all of them can, and the one
...
and scores 0.835. **On the original numbers I'd have thrown out a competitive model for being
...
model that falls back to the largest category whenever it's unsure is penalised for ignoring
the other five. My answer key contains 173 bugs and only 5 issues in the "other" category, so
this is a real weakness rather than a theoretical one. **That's why macro-F1 is my headline
...
every model's score. In production I'd filter them out with a text pattern and never pay a
...
one category wouldn't add information. It would add noise, which would then penalise whichever
...
work, and I'd have lost it entirely if I had forced those 15 issues into categories.
...
deliberately looking for the categories that the maintainers' labels can't supply. Every label
I wrote has a written reason recorded beside it.

**One category can't honestly be measured.** The "other" category has only 3 issues in the
final scoring set, so a single issue moves its score by about 0.1. The application marks any
category with fewer than 10 examples as "thin". I'd rather state plainly that a category
can't be measured than invent examples to make the number look respectable.
...
group**. Had I not caught that, I'd have been reporting the models' memory of examples they
...
rules it out for four reasons I'd have arrived at independently. You lose the timing for each
individual issue. You can't retry one failed issue without repeating the whole group. One badly
formed reply can corrupt the issues grouped with it. And you can't send a single difficult issue
...
Before going further I'd want the level of agreement measured **for each product
separately**, not averaged across all of them. Say the new model agrees with the current system on 88% of
doctl issues but only 61% of issues from a database product. That means the categories mean
something different in that second product. Changing models won't fix it. This
stage isn't optional, and the reason is worth spelling out. doctl is a command-line tool, so
...
and it's the most likely reason for a choice that looked good on doctl to perform badly
elsewhere.

**Build a separate answer key for each product** while shadow mode runs. I'd aim for 150 to
250 issues per product, using the same two-source method I used here. This is where the real cost
of the project sits, and it's roughly one day of an engineer's attention per product rather
than money spent on computing. Before going further I'd want two people to label the same
100 issues, so that there's a measured figure for how often people agree with each other. That
...
categorised issue is easy to correct while a missed vulnerability isn't.
...
**Where I expect this to break down.** Some issues are genuinely ambiguous. That's a permanent
feature of the workload, not a flaw in my labelling, so those should go to a person. The most
useful thing a classifier can say about an ambiguous issue is that it won't guess. Six categories won't survive contact with six products. Somebody will
eventually need a `billing` category. So the list of categories lives in one place, and the
prompt is generated from that list rather than typed out by hand. The
way people write issues drifts over the years, so I'd monitor how the spread of predicted
...
for that event. Providers also update models without changing their names, so I'd replay a
small fixed set of issues on a schedule and compare the results against the saved run for the
same dataset. Finally I'd cap the output length and alarm on the average number of
output tokens per call. Total spending only tells you about a problem after the money is gone.
Average output length tells you which model changed, and when.

**I wouldn't fine-tune a model yet.** I'd only consider it once a straightforward prompted
model, together with the second opinion, has stopped improving and is still performing below the
level at which people agree with each other. That level is precisely the thing I haven't
...
**Settings.** `DO_INFERENCE_API_KEY` holds the Serverless Inference API key, and it's the only
setting you must provide. It's never built into the image. Every other setting has a working
default.
...
there's no rate limit. An unprotected public URL could therefore drain the credit balance in
...
I should be clear about what this isn't. Basic auth gives one shared username and password. It
provides no user accounts, no per-caller spending limit, and no record of who ran what. For a
single-user evaluation tool served over HTTPS that's the right amount of security. For
anything with several users it wouldn't be.
...
not survive. That's worth knowing before you conclude the run list is broken. Runs made on a
...
**I didn't compare against a frontier model.** The credits don't cover Claude or GPT-5.x, so
...
differences were inside run-to-run noise. That doesn't prove a frontier model would add nothing.
It does mean nothing in this field showed a size-related advantage worth paying for.

**I didn't have a second person check the answer key.** One person wrote it in a single pass,
so I have no measurement of how often two people would agree. This is the answer key's greatest
weakness and the first thing I'd correct at a larger scale.

**I didn't check whether the confidence scores mean anything.** I record them for every call
...
something I've measured.

**I didn't fine-tune, use retrieval, or combine several models by voting.** All three are
...
**I didn't label all 536 issues by hand.** 174 issues have no correct answer recorded and
...
**I didn't use scikit-learn.** Precision, recall and F1 across six categories takes about
twenty lines to write, and the exercise asks for the arithmetic to be checkable. You can read my
definition of F1 in `metrics.py`. You can't read one that sits inside an imported library.
